...
export default function KycStatusManager() {
  ...
  useEffect(() => {
    fetch("/api/kyc")
      ...
      .then((data) => {
        ...
        // AUTO-OPEN: Only show if NOT approved
  ...
  // Logic: Approved ya Loading state mein component hide rahega
  if (loading || kyc.status === "APPROVED") return null;
  ...
  return (
    <div className="font-sans">
      {/* 1. FLOATING TOGGLE BUTTON (Z-INDEX 9998) */}
      {!isOpen && (
        <button
      ...
      {/* 2. OVERLAY & CENTERED POPUP (Z-INDEX 9999) */}
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-[380px] bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            {/* TOP RIGHT CLOSE ICON (Hidden on very small screens to keep it clean) */}
            <button 
